...
_.extend(AudioParam.prototype, {
  ...
  _tick: function() {
    DspObject.prototype._tick.apply(this, arguments)
    var buffer = []
    this._dsp(buffer)
    return buffer
  },
  ...
  _to_aRate_linearRamp: function(target, endTime) {
    var blockSize = AudioParam.BLOCK_SIZE
      ...
      , step = (target - U0) / (endTime - startTime) * 1 / this._context.sampleRate
  ...
  _to_kRate_linearRamp: function(target, endTime) {
    var blockSize = AudioParam.BLOCK_SIZE
      , U0 = this._instrinsicValue, Un = U0
      , startTime = this._context.currentTime
      , step = (target - U0) / (endTime - startTime) * blockSize / this._context.sampleRate
      , next = this._arithmeticSeries(U0, step)
      , i
  ...
  _to_aRate_exponentialRamp: function(target, timeEnd) {
    var timeStart = this._context.currentTime
      , blockSize = AudioParam.BLOCK_SIZE
      , U0 = this._instrinsicValue, Un = U0
      , ratio = Math.pow(target / U0, 1 / (this._context.sampleRate * (timeEnd - timeStart)))
  ...
  _to_kRate_exponentialRamp: function(target, timeEnd) {
    var timeStart = this._context.currentTime
      , blockSize = AudioParam.BLOCK_SIZE
      , U0 = this._instrinsicValue, Un = U0
      , ratio = Math.pow(target / U0, blockSize / (this._context.sampleRate * (timeEnd - timeStart)))
      , next = this._geometricSeries(U0, ratio)
      , i
  ...
  _to_aRate_setTarget: function(target, Tc, done) {
    var timeStart = this._context.currentTime
      ...
      , ratio = Math.exp(-(1 / this._context.sampleRate) / Tc)
      , next = this._geometricSeries(U0, ratio)
      , blockSize = AudioParam.BLOCK_SIZE
  ...
  _to_kRate_setTarget: function(target, Tc, done) {
    var timeStart = this._context.currentTime
      , blockSize = AudioParam.BLOCK_SIZE
      , U0 = this._instrinsicValue - target, Un = target + U0
      , ratio = Math.exp(-(blockSize / this._context.sampleRate) / Tc)
      , next = this._geometricSeries(U0, ratio)
      , i
  ...
  _to_kRate_SetValueCurve: function(values, startTime, duration, done) {
    ...
    this._dsp = function(buffer) {
      val = values[Math.min(Math.round(coeff * (this._context.currentTime - startTime)), valuesLength - 1)]
