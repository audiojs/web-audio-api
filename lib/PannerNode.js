...
const fixNANs = x => Number.isFinite(x) ? x : 0
...
class PannerNode extends AudioNode {
  ...
  _calculateAzimuthElevation() {
    ...
    azimuth = fixNANs(azimuth)  // avoid illegal values
    ...
    // Elevation
    let elevation = 90 - rad2deg(sourceListener.angleBetween(up))  // double
    elevation = fixNANs(elevation) // avoid illegal values
