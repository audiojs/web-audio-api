...
var AudioBufferSourceNode = module.exports = function(context) {
  ...
  this._pullAudio = this._pullAudioZeros
}
...
_.extend(AudioBufferSourceNode.prototype, {
    ...
    this._schedule('start', when, function() {
      ...
      self._pullAudio = function(done) {
        ...
        if (cursorNext < cursorEnd) {
          ...
          } else self._pullAudio = self._pullAudioZeros
          cursor = cursorNext
          done(null, outBuffer)
        }
      }
  ...
  stop: function(when) {
    ...
    this._schedule('stop', when, function() {
      self._pullAudio = self._pullAudioZeros
    })
  },

  pullAudio: function(done) {
    // We run tick first, in case 'start' or 'stop' is scheduled and changes
    // the pullAudio method
    this._tick()
  ...
  _pullAudioZeros: function(done) {
    // We don't run _tick here, because in WAA this node doesn't work after having been used.
    done(null, new AudioBuffer(1, constants.BLOCK_SIZE, this.context.sampleRate)) 
  }
