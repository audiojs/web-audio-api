      ...
      self._dsp = function() {
        ...
        if (cursorNext < cursorEnd) {
          ...
          if (self.loop) {
            ...
            if (self.onended){
              self._schedule('onended', self.context.currentTime + (cursorNext - cursorEnd)/sampleRate, self.onended);
            }
           self._schedule('kill', self.context.currentTime + (cursorNext - cursorEnd)/sampleRate, self._kill, undefined, self);
          }
          cursor = cursorNext
          return outBuffer
        }
