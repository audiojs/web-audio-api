      ...
      self._dsp = function() {
        ...
        if (cursorNext < cursorEnd) {
          ...
          if (self.loop) {
            ...
            var time = self.context.currentTime + (cursorNext - cursorEnd)/sampleRate
            if (self.onended) self._schedule('onended', time, self.onended)
            self._schedule('kill', time, self._kill.bind(self))
          }
          cursor = cursorNext
          return outBuffer
        }
