    ...
    inBuffers.forEach(function(inBuffer) {
      ...
      if (inBuffer.numberOfChannels === computedNumberOfChannels) {
        for (ch = 0; ch < computedNumberOfChannels; ch++)
          outBuffer.getChannelData(ch).set(inBuffer.getChannelData(ch))
      ...
      // Down-mixing the source
        ...
        if (channelInterpretation === 'discrete') discreteDownMix()
          ...
          var inChannels = inBuffer.numberOfChannels
            , outChannels = computedNumberOfChannels
            ...
            case 1:
              switch (outChannels) {
                ...
                case 2:
                  ...
                  for (ch = 0; ch < 2; ch++)
                    outBuffer.getChannelData(ch).set(inData)
                  break
                ...
                case 6:
                  outBuffer.getChannelData(2).set(inBuffer.getChannelData(0))
                  break
              ...
              switch (outChannels) {
                ...
                case 6:
                  for (ch = 0; ch < 2; ch++)
                    outBuffer.getChannelData(ch).set(inBuffer.getChannelData(ch))
                  for (ch = 2; ch < 4; ch++)
                    outBuffer.getChannelData(ch + 2).set(inBuffer.getChannelData(ch))
                  break
