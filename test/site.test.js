...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  is(document.querySelector('.hero-lede').textContent, 'Your web-audio code, cross-platform. No native bindings, no compilation. Process audio in batch, unit-test it in CI, stream from servers, render at the edge, script it in CLI. Any JS engine.')
  ...
  is([...document.querySelectorAll('.hero-stack li')].map(node => node.textContent.trim()).join('|'), 'Node|Deno|Bun|Porffor', 'runtime stack rides the hero foot')
  ...
  ok(document.querySelector('.site-footer .wpt-badge img[src^="https://img.shields.io/github/actions/workflow/status/audiojs/web-audio-api/wpt.yml"]'), 'the live W3C WPT badge rides the footer')
  ...
  is(document.querySelectorAll('header a').length, 6, 'brand, section nav, version, WPT badge, and GitHub')
  ...
  for (let fact of ['54 KB gzipped', 'TypeScript', 'MIT']) ok(document.querySelector('header .spec-facts').textContent.includes(fact), `header carries ${fact}`)
  ...
  ok(document.querySelector('header .corner-links .wpt-badge img[src^="https://img.shields.io/github/actions/workflow/status/audiojs/web-audio-api/wpt.yml"]'), 'the black W3C WPT badge rides the header')
