...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  is(document.querySelectorAll('header a').length, 6, 'brand, section nav, version, GitHub, and AudioJS')
  ...
  for (let fact of ['54 KB gzipped', 'TypeScript', 'MIT', '2013']) ok(document.querySelector('.site-footer').textContent.includes(fact), `footer carries ${fact}`)
  ...
  ok(document.querySelector('header .corner-links a[href="https://audiojs.dev/"] svg path[d^="M28.6572"]'), 'the audiojs org mark rides the right corner')
