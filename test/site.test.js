...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  ok(/\.hero-signal \.demo-runbar \{[^}]*background-color: var\(--color-ink\)/s.test(sheetCss), 'the playback row stands on the panel\'s own ground, off the ruled field')
  ...
  let detailBlocks = ['detail-head', 'detail-grid', 'detail-foot', 'callout', 'detail-related', 'detail-seo']
...
test('every example has a crawlable canonical detail page', () => {
  ...
  for (let example of examples) {
    ...
    ok(document.querySelector('.detail-grid + .detail-foot .demo-runbar #demo-run'), `${example.id} plays from the same footer as the modal`)
    ...
    if (callouts.length) ok(callouts[0].previousElementSibling.classList.contains('detail-foot') && callouts[0].querySelector('.callout-label').textContent === (example.warning ? 'Warning' : 'Note'), `${example.id} callout follows the stage`)
  }
})
...
test('the homepage wires the hero play, the graph tab, and the page-side Web Audio', () => {
  ...
  ok(document.querySelector('#example-dialog .dialog-shell > .dialog-foot .demo-runbar #demo-run'), 'the one action the modal is for closes it as its footer')
  ...
  // the panels cost a phone as little as they can: one blit per column, bytes from the analyser,
  // colours read once, and every sample not needed for a peak skipped
  ...
  ok(browser.includes('envelopeOf(samples, 0, samples.length, 4)'), 'the envelope strides its samples')
