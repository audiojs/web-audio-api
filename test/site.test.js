...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  ok(!dialog.querySelector('.demo-provenance') && document.querySelector('.hero-demo-note #hero-engine'), 'the hero chooses its engine without repeating provenance below every example')
...
test('the homepage wires the hero play, the graph tab, and the page-side Web Audio', () => {
  ...
  ok(caption?.querySelector('#hero-engine') && caption.nextElementSibling.classList.contains('hero-code'), 'the preview note belongs just above the code, not inside the graph')
  is(caption.querySelector('#hero-engine').getAttribute('aria-checked'), 'true', 'the package engine is the default')
  is(caption.querySelector('#hero-engine').getAttribute('role'), 'switch', 'engine selection has a keyboard-accessible switch')
  ...
  ok(shim.includes('this.context instanceof PreviewContext') && shim.includes('this.nodes.clear()'), 'feedback retention belongs only to hero contexts and ends on close')
...
function deadRules(sheet, pages, scripts) {
  ...
  let skeleton = selector => selector
    ...
    .replace(/::[a-z-]+/gi, '')
    .replace(/:(hover|focus-visible|focus-within|focus|active|disabled|checked|open|target|visited|empty|placeholder-shown|invalid|valid|required|enabled|link|any-link|popover-open|modal|defined)\b/gi, '')
    .replace(/\[[^\]]*\]/g, '').trim()
