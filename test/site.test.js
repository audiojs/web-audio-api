...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  is(document.querySelector('.hero-lede').textContent, 'Your web-audio code, cross-platform. No native bindings, no compilation. Process audio in batch, unit-test it in CI, stream from servers, render at the edge, script it in CLI. Any JS:')
  ...
  is([...document.querySelectorAll('.hero-stack li')].map(node => node.textContent.trim()).join('|'), 'Node.js|Deno|Bun|Porffor|JZ', 'runtime stack rides the hero foot')
  is([...document.querySelectorAll('.hero-stack button[data-engine]')].map(node => node.dataset.engine).join('|'), 'node|deno|bun|porffor|jz', 'every engine is a selectable pill')
  ...
  ok(document.querySelector('.site-footer .wpt-badge img[src$="workflows/wpt.yml/badge.svg"]'), 'the live W3C WPT workflow badge rides the footer')
  ok(read('.github/workflows/wpt.yml').includes('name: W3C WPT'), 'the dedicated WPT workflow names the badge')
  ok(document.querySelector('.site-footer .wpt-badge[href="https://github.com/audiojs/web-audio-api/actions/workflows/wpt.yml"]'), 'WPT badge links to its workflow')
  ...
  for (let fact of ['208 KB', 'TypeScript', 'MIT', '2013']) ok(document.querySelector('.site-footer').textContent.includes(fact), `footer carries ${fact}`)
  ok(document.querySelector('.site-footer .footer-brand[href="https://audiojs.dev/"] svg path[d^="M3.72998 24.4174"]'), 'footer wave is the audiojs.dev logo path')
  ok(document.querySelector('.site-footer .footer-brand svg path[d="M3.355 24.3584h41.29"]'), 'footer wave sits on the logo baseline')
  ok(document.querySelector('.site-footer a[href="https://github.com/sponsors/audiojs"]'), 'footer invites support')
  ok(!document.querySelector('.site-footer').textContent.includes('web-audio-api'), 'footer does not repeat the package name')
