...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  ok(document.querySelector('.hero-lede').textContent.trim().startsWith('Your web-audio code cross-platform. No native bindings, no compilation. Process audio in batch, unit-test it in CI, stream from servers, render at the edge, run Tone.js or other lib headless, script it in CLI.'), 'lede keeps the exact claim ahead of the engine pills')
  ...
  is([...document.querySelectorAll('.hero-lede .hero-stack button')].map(node => node.textContent.trim()).join('|'), 'Node|Deno|Bun|Porffor|LLRT', 'named engine pills ride inline with the lede')
  is([...document.querySelectorAll('.hero-stack button[data-engine]')].map(node => node.dataset.engine).join('|'), 'node|deno|bun|porffor|llrt', 'every engine is a selectable pill')
