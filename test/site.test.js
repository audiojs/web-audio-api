...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  ok(document.querySelector('.hero-spec .wpt-badge[href="https://github.com/audiojs/web-audio-api/actions/workflows/wpt.yml"]'), 'WPT badge links to its workflow')
  ...
  for (let fact of ['54 KB gzipped', 'TypeScript', 'MIT']) ok(document.querySelector('.hero-spec').textContent.includes(fact), `hero spec plate carries ${fact}`)
  ...
  ok(document.querySelector('.hero-spec .wpt-badge img[src^="https://img.shields.io/badge/W3C%20WPT-100%25"]'), 'the 100% W3C WPT badge rides the spec plate')
  ok(document.querySelector('.site-footer a[href="https://github.com/sponsors/audiojs"]'), 'footer invites support')
  is(document.querySelector('.site-footer .footer-name').textContent, `web-audio-api v${pkg.version}`, 'footer names the package with its version')
  is(document.querySelector('.site-footer .footer-install[href="https://www.npmjs.com/package/web-audio-api"]').textContent, 'npm install web-audio-api', 'footer install command leads to npm')
