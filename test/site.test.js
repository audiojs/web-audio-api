...
test('homepage is only the hero, example catalogue, compact FAQ, and footer', () => {
  ...
  is(document.querySelector('.hero-lede').textContent, 'Your web-audio code, running cross-platform. No native bindings, no compilation. Process audio in batch, unit-test it in CI, stream from servers, render at the edge, script it in the terminal. Any JS:')
  ...
  ok(document.querySelector('.site-footer .footer-brand[href="https://audiojs.dev/"] svg path[stroke="currentColor"]'), 'footer carries the AudioJS wave')
