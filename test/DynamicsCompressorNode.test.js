...
test.mute('DynamicsCompressorNode > compresses loud signal', () => {
  ...
  wire(c, node, AudioBuffer.filledWithVal(1.0, 1, BLOCK_SIZE, SR))
  let buf
...
test.mute('DynamicsCompressorNode > attack/release timing', () => {
  let c = { sampleRate: SR, currentTime: 0 }
  let node = new DynamicsCompressorNode(c)
  let src = wire(c, node, AudioBuffer.filledWithVal(1.0, 1, BLOCK_SIZE, SR))
  ...
  src._tick = () => AudioBuffer.filledWithVal(0, 1, BLOCK_SIZE, SR)
